...
export default {
   ...
   * @param {Number} height
   */
  async fetchValidatorsetsFromHeight({
      commit,
    },
    height
  ) {
  ...
  async getValidatorData({
    ...
    try {
      ...
      if (state.details.description.identity != "") {
        await dispatch("fetchValidatorPicture", state.details.description.identity);
      }
    } catch (error) {
      if (error.response) {
        commit("setError", JSON.stringify(error.response.data));
