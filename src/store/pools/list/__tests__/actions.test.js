...
describe('store/pools/list/actions', () => {
  beforeEach(() => {
    mockError = false;
    mockResponse = null;
    mockNextKey = '';
  ...
  test('if "initPoolsList" reset store, set loading state, dispatch wanted actions', async () => {
    const commit = jest.fn();
    const dispatch = jest.fn();

    await actions.initPoolsList({ commit, dispatch });
  ...
  test('if "getPoolsContracts" return the contracts, and set the error if it is caught', async () => {
    const commit = jest.fn();

    const response = await actions.getPoolsContracts({ commit });

    expect(response.length).toBe(mockContractsCount * mockResponseCount);

    mockError = true;

    await actions.getPoolsContracts({ commit });
